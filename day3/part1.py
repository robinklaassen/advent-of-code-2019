...
def draw_path(vectors: Tuple[str, ...],
    ...
    grid = np.zeros((grid_size_1d, grid_size_1d), dtype=np.bool)

    current_row = axis_size_from_origin
    current_column = axis_size_from_origin
    grid[current_row, current_column] = 1  # mark origin

    for vector in vectors:
        letter, length = parse_vector(vector)

        if letter == 'R':
            grid[current_row, current_column+1:current_column+length+1] = 1
            current_column += length
        elif letter == 'L':
            grid[current_row, current_column-length:current_column] = 1
            current_column -= length
        elif letter == 'D':
            grid[current_row+1:current_row+length+1, current_column] = 1
            current_row += length
        elif letter == 'U':
            grid[current_row-length:current_row, current_column] = 1
            current_row -= length
...
def get_closest_intersection_distance(x1: Tuple[str, ...],
    ...
    for row_id, col_id in list(zip(row_idx, col_idx)):
        distance = abs(row_id - axis_size_from_origin) + abs(col_id - axis_size_from_origin)
        distances.append(distance)
    distances.remove(0)  # remove origin
...
def draw_dummy():
    draw_path(("R2", "U2", "L4", "D4"), axis_size_from_origin=2, print_grids=True)
...
def test_case_1():
    ...
    assert get_closest_intersection_distance(path1, path2) == 6
...
def test_case_2():
    ...
    assert get_closest_intersection_distance(path1, path2, axis_size_from_origin=500) == 159
...
if __name__ == "__main__":
    ...
    result = get_closest_intersection_distance(path1, path2, axis_size_from_origin=20_000)
