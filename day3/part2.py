...
def draw_path(vectors: Tuple[str, ...],
              axis_size_from_origin: int = DEFAULT_AXIS_SIZE_FROM_ORIGIN,
              print_grids: bool = False) -> np.ndarray:
    grid_size_1d = 2 * axis_size_from_origin + 1
    grid = np.zeros((grid_size_1d, grid_size_1d), dtype=np.int64)

    current_row = axis_size_from_origin
    current_column = axis_size_from_origin

    total_length = 0
    for vector in vectors:
        letter, length = parse_vector(vector)

        added_numbers = np.arange(length) + total_length + 1

        if letter == 'R':
            slice_ = grid[current_row, current_column+1:current_column+length+1]
            new_numbers = np.where(slice_ == 0, added_numbers, slice_)
            grid[current_row, current_column+1:current_column+length+1] = new_numbers
            current_column += length
        elif letter == 'L':
            slice_ = grid[current_row, current_column-length:current_column]
            new_numbers = np.where(slice_ == 0, added_numbers[::-1], slice_)
            grid[current_row, current_column-length:current_column] = new_numbers
            current_column -= length
        elif letter == 'D':
            slice_ = grid[current_row+1:current_row+length+1, current_column]
            new_numbers = np.where(slice_ == 0, added_numbers, slice_)
            grid[current_row+1:current_row+length+1, current_column] = new_numbers
            current_row += length
        elif letter == 'U':
            slice_ = grid[current_row-length:current_row, current_column]
            new_numbers = np.where(slice_ == 0, added_numbers[::-1], slice_)
            grid[current_row-length:current_row, current_column] = new_numbers
            current_row -= length

        if print_grids:
            print(f"Grid after vector: {vector}")
            print(grid)
...
def get_closest_intersection_distance(x1: Tuple[str, ...],
                                      x2: Tuple[str, ...],
                                      axis_size_from_origin: int = DEFAULT_AXIS_SIZE_FROM_ORIGIN) -> int:
    grid1 = draw_path(x1, axis_size_from_origin=axis_size_from_origin)
    grid2 = draw_path(x2, axis_size_from_origin=axis_size_from_origin)
    crosses = np.logical_and(grid1, grid2)
    row_idx, col_idx = np.where(crosses)

    distances = []
    for row_id, col_id in list(zip(row_idx, col_idx)):
        distance = grid1[row_id, col_id] + grid2[row_id, col_id]
        distances.append(distance)
    print(f"Found {len(row_idx) - 1} intersections with distances: {distances}")
    print(f"Lowest distance is {min(distances)}")
    return min(distances)


def draw_dummy():
    draw_path(("R3", "U3", "L2", "D5"), axis_size_from_origin=3, print_grids=True)
...
def test_case_1():
    ...
    assert get_closest_intersection_distance(path1, path2) == 30


def test_case_2():
    print("---TEST CASE 2---")
    path1 = tuple("R75, D30, R83, U83, L12, D49, R71, U7, L72".split(", "))
    path2 = tuple("U62, R66, U55, R34, D71, R55, D58, R83".split(", "))
    assert get_closest_intersection_distance(path1, path2, axis_size_from_origin=500) == 610
...
if __name__ == "__main__":
    ...
    print("---MAIN PUZZLE---")
    path1 = tuple(PUZZLE_INPUT_1.split(","))
    path2 = tuple(PUZZLE_INPUT_2.split(","))
    result = get_closest_intersection_distance(path1, path2, axis_size_from_origin=15_000)
    print(f"Submit answer: {result}")
